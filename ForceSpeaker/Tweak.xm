...
static BOOL FSHasExternalRoute(AVAudioSession *session) {
    ...
    for (AVAudioSessionPortDescription *port in route.outputs) {
        ...
        if ([type isEqualToString:AVAudioSessionPortHeadphones] ||
            ...
            [type isEqualToString:AVAudioSessionPortAirPlay]) {
            return YES;
        }
    }
    return NO;
...
static void FSForceSpeaker(AVAudioSession *session) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        // Do not steal an explicitly connected external audio device.
        if (FSHasExternalRoute(session)) return;

        NSError *error = nil;
        AVAudioSessionPortOverride result =
            [session overrideOutputAudioPort:AVAudioSessionPortSpeaker error:&error];
